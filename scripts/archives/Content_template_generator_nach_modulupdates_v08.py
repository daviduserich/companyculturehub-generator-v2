...
CSV_LAYOUT_PATH = find_latest_file(CSV_LAYOUT_PATTERN) or BASE_DIR / 'content_template_new_project/layout_extended_v2.csv'

GLOBAL_VARS_PATTERN = 'templates/components/global_variables*.csv'  # NEU
GLOBAL_VARS_PATH = find_latest_file(GLOBAL_VARS_PATTERN) or BASE_DIR / 'templates/components/global_variables.csv'

LOCAL_VARS_PATTERN = 'templates/components/local_variables*.csv'  # NEU
LOCAL_VARS_PATH = find_latest_file(LOCAL_VARS_PATTERN) or BASE_DIR / 'templates/components/local_variables.csv'
...
def generate_content_template():
    ...
    for row in layout:
        ...
        if not component:
            continue
        # NEU: Ignoriere enabled=FALSE nicht vollständig, aber prüfe min_count > 0 für Generierung
        if int(row.get('min_count', 0)) == 0:
            logging.info(f"Komponente '{component}' hat min_count=0 und wird übersprungen.")
            continue
