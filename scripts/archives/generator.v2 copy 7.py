#!/usr/bin/env python3
"""
CompanyCultureHub Site Generator v2.6 (Final Data Access Fix)
"""
...
class CompanyCultureHubGenerator:
    ...
    def generate_site(self, project_name):
        print(f"🚀 CompanyCultureHub Generator v2.6 gestartet für Projekt: {project_name}")
        ...
        for item in layout_plan:
            ...
            # Ersetze alle einfachen Platzhalter in dieser Komponente
            component_html = self._replace_placeholders(component_html, final_mapping)
            
            # Hole die spezifischen Daten für diese Sektion (inkl. Listen)
            # KORREKTUR: Greife auf die Sektion zu, die in der CSV definiert ist
            section_data = page_content_data.get(data_source_key, [])
            
            # Verarbeite NUR die Listen, die zu dieser Sektion gehören
...
def main():
    ...
    parser = argparse.ArgumentParser(description='CompanyCultureHub Site Generator v2.6')
