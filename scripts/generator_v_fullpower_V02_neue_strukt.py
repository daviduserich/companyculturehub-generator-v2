...
def main():
    ...
    # === PHASE 1: AUFRÄUMEN (MIT ROBUSTER ARCHIVIERUNG) ===
    print("\n[PHASE 1: AUFRÄUMEN]")
    docs_archive_dir = OUTPUT_DIR / "docs_archives"
    docs_archive_dir.mkdir(exist_ok=True)
    
    # KORREKTUR: Robuste Archivierung für HTML-Dateien
    for f in OUTPUT_DIR.glob('*.html'):
        destination_path = docs_archive_dir / f.name
        # Wenn die Datei im Archiv bereits existiert, benenne sie um
        if destination_path.exists():
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            new_name = f"{f.stem}_{timestamp}{f.suffix}"
            destination_path = docs_archive_dir / new_name
        
        shutil.move(str(f), str(destination_path))
        logging.info(f"Archiviere alte HTML-Datei nach: {destination_path.name}")
    ...
    content_archive_dir.mkdir(exist_ok=True)
        ...
        # KORREKTUR 2: Ignoriere den Archiv-Ordner selbst bei der Suche
    for folder in CONTENT_DIR.glob('processed_*'):
        if folder.name == content_archive_dir.name:
            continue # Überspringe den Archiv-Ordner

        destination_path = content_archive_dir / folder.name
        # Wenn der Ordner im Archiv bereits existiert, füge einen Timestamp hinzu
        if destination_path.exists():
            timestamp = datetime.now().strftime("%Y%m%d_%H%M%S")
            new_name = f"{folder.name}_{timestamp}"
            destination_path = content_archive_dir / new_name

        shutil.move(str(folder), str(destination_path))
        logging.info(f"Archiviere verarbeitetes Projekt nach: {destination_path.name}")
    print("   - ✅ Verarbeitete Projekte archiviert.")

    # === PHASE 2: PROJEKTE VERARBEITEN (unverändert) ===
    ...
    placeholder_assets = load_json(TEMPLATES_DIR / "components" / "_placeholder_assets.json", {})

    for project_dir in projects_to_process:
        project_name = project_dir.name
        print(f"🚀 Verarbeite Projekt: {project_name}")

        # Finde die neueste Content-Datei (jetzt Stufe02)
        content_pattern = "project_template_Stufe02_Styled_*.json"
        all_styled_files = list(project_dir.glob(content_pattern))
        
        if not all_styled_files:
            logging.warning(f"Keine '{content_pattern}' Dateien in {project_name} gefunden. Überspringe.")
            continue
        ...
        # Führe die Generierung für jede gefundene Stufe02-Datei aus
        for content_file in all_styled_files:
            content_data = load_json(content_file, {})
            if not content_data: continue
            
            # Extrahiere den Style-Namen aus dem Dateinamen
            style_name_match = re.search(r"_Styled_(.+?)_\d{8}", content_file.name)
            if not style_name_match: continue
            style_name = style_name_match.group(1)

            generate_site_for_style(project_dir, style_name, content_data, layout_plan, placeholder_assets)

        # === PHASE 3: PROJEKT MARKIEREN (unverändert) ===
