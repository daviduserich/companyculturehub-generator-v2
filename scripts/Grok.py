#!/usr/bin/env python3
"""
CompanyCultureHub HTML Generator v19.2 (The Complete Modules Edition)
- Basierend auf allen HTML- und JSON-Modulen (14 Module, 53+ Platzhalter).
- Fallbacks: value > example_value > description > default.
- Konsolidiert <style>-Blöcke im <head>, wrappt :root.
- Merges class-Attribute, validiert Ersetzungen.
- Handhabt Listen mit Nested Fallbacks.
...
def load_json(file_path, default=None):
    ...
    try:
        with file_path.open(encoding='utf-8') as f: return json.load(f)
...
def replace_placeholders(html, data_dict):
    ...
    if remaining: logging.warning(f"Unersetzte: {remaining}")
    return html
...
def generate_site_for_style(project_dir, style_name, content_data, layout_plan, placeholder_assets, global_defaults):
    ...
    if "global_settings" in content_data: final_mapping.update(flatten_dict(content_data["global_settings"]))
    ...
    for item in sorted(layout_plan, key=lambda x: int(x.get('order', 0))):
        ...
        for key, data in instance_data.items():
            if isinstance(data, dict):
                ...
                final_mapping[f"{component_name}.{key}"] = data  # Später verarbeiten
        ...
        for key in ["image_url", "map_url", "image_alt_text"]:
            ...
            if not final_mapping.get(full_key):
                ...
                if urls: final_mapping[full_key] = urls[instance_index % len(urls)]
...
def main():
    print("--- STARTING HTML GENERATOR V19.2 ---")
    # Aufräum-Code (wie zuvor)
    ...
    for f in OUTPUT_DIR.glob('*.html'):
        ...
        if destination_path.exists(): destination_path = destination_path.with_name(f"{f.stem}_{datetime.now().strftime('%Y%m%d_%H%M%S')}{f.suffix}")
    ...
    if folders_to_archive:
        for folder in folders_to_archive:
            ...
            if destination_path.exists(): destination_path = destination_path.with_name(f"{folder.name}_{datetime.now().strftime('%Y%m%d_%H%M%S')}")
...
if __name__ == "__main__":
    ...
    if not defaults_path.exists():
        defaults = {
            # Erweiterte Defaults basierend auf allen Modulen
            ...
            # Füge weitere für alle Module hinzu, wenn nötig
        }
