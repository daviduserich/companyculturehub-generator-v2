...
def main():
    ...
    try:
        ...
        project_dir = "/home/ubuntu/content/DEF_88"
        templates_dir = "/home/ubuntu/templates"
        components_dir = "/home/ubuntu/templates/components"
        output_dir = "/home/ubuntu/docs"
    ...
    # Schritt 2: Automatische Stil-Erkennung
    try:
        print("\n🔍 Schritt 2: Automatische Stil-Erkennung...")
        
        # Suche nach interpreted_styles_*.json Dateien
        style_pattern = os.path.join(project_dir, "interpreted_styles_*.json")
        style_files = glob.glob(style_pattern)
        
        print(f"   🔎 Suche nach: {style_pattern}")
        print(f"   📄 Gefundene Dateien: {len(style_files)}")
        
        if not style_files:
            print("💥 FATALER FEHLER: Keine 'interpreted_styles_*.json' Dateien im Projektordner gefunden. Kann keine Stile generieren.")
            return False
        
        # Extrahiere Stil-Namen aus Dateinamen (mit Duplikat-Vermeidung)
        available_styles = []
        seen_styles = set()
        
        for file_path in style_files:
            filename = os.path.basename(file_path)
            # Extrahiere Stil-Namen zwischen "interpreted_styles_" und "_"
            match = re.search(r'interpreted_styles_([^_]+)_', filename)
            if match:
                style_name = match.group(1)
                if style_name not in seen_styles:
                    available_styles.append(style_name)
                    seen_styles.add(style_name)
                    print(f"   ✓ Stil erkannt: {style_name} ({filename})")
                else:
                    print(f"   ⚠️  Stil bereits erkannt, überspringe: {style_name} ({filename})")
        
        if not available_styles:
            print("💥 FATALER FEHLER: Keine gültigen Stil-Namen aus Dateinamen extrahiert.")
            return False
    ...
    try:
        ...
        # Lade HTML-Sammlung
        html_components_file = "/home/ubuntu/upload/en_bloc_components_HTML_20250904_180504.md"
        json_components_file = "/home/ubuntu/upload/en_bloc_components_JSON_20250904_180504.md"
    ...
    for style_name in available_styles:
        ...
        try:
            ...
            # Header laden (falls vorhanden)
            header_path = os.path.join(templates_dir, "technik_header.html")
...
<body class="{{BODY_CLASSES}}">"""
                print(f"   ⚠️  Header-Datei nicht gefunden, verwende Fallback")
